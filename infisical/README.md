...
> **Status:** this mechanism is opt-in (`docker compose --profile rotation
up -d`) and currently self-contained — `api` does **not** consume it.
> `api` still gets `app_user`'s password from the plain Compose secret at
> `secrets/db_password` (see the root README's "Rotate the DB password"
> section). Everything below describes how the sidecar itself works when
> you do bring it up; wiring `api` onto it is a later step.

Infisical Cloud is the source of truth for `app_user`'s Postgres password,
whenever this profile is running. `on-change.sh` reads the Agent's rendered
file (`/shared-secrets/db-password`, kept current by `infisical-agent` — a
sidecar container) and syncs Postgres to match. Nothing in `api` talks to
Infisical, has Infisical credentials, or reads that file today.
...
postgres starts → init.sql runs (fresh volume only) → postgres healthy
  → infisical-agent starts (only with --profile rotation):
...
`infisical-agent`'s healthcheck doesn't just check a file exists, it tries
to connect with it — so the container never reports healthy against a
password that doesn't actually work yet. Nothing currently depends on that
healthcheck (`api` starts independently, straight after `postgres`), but
it's what you'd gate a future `api` dependency on if it's ever wired up.
...
```bash
docker compose --profile rotation up --build -d --wait
```
